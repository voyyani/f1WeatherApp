...
def analyze_weather(year, gp, driver, session_type):
    try:
        ...
        return {
            'success': True,
            'plot_url': 'data:image/png;base64,' + base64.b64encode(buf.getvalue()).decode('utf-8'),  # Fixed key and format
            'temp_corr': float(temp_corr),
            'rain_corr': float(rain_corr) if rain_corr is not None else None
        }
